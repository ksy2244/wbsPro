...
public class CooperationDAOImpl implements CooperationDAO {
	...
	public int cooperUpdate(CooperationDTO dto) throws SQLException {
		...
		try {
			...
			sql = "UPDATE COOPERATION SET coo_name = ?, coo_tel = ?, " + "coo_manager = ? WHERE coo_code = ?";
	...
	public int cooperEmpUpdate(CooperationDTO dto) throws SQLException {
		...
		try {
			...
			sql = "UPDATE COOUSER SET WORK_START_DATE = ?, WORK_END_DATE = ?, " + " WHERE COO_USER_CODE = ?";
	 ...
	 * 업체 사원 이름 없어 보류
	 * 
	 * @Override public int cooperEmpDeleteName(String cooperEmpName) throws
	 * SQLException { int result = 0; PreparedStatement pstmt = null; String sql;
	 * 
	 * try {
	 * 
	...
	public List<CooperationDTO> cooperSearchCode(String cooperCode) {
		...
		try {
			...
			sql = "SELECT cu.COO_CODE, COO_NAME, COO_TEL, COO_MANAGER, TO_CHAR(WORK_START_DATE, 'YYYY-MM-DD') WORK_START_DATE, "
					+ " TO_CHAR(WORK_END_DATE,'YYYY-MM-DD') WORK_END_DATE, USER_CODE " + " FROM COOPERATION cp"
					+ " LEFT OUTER JOIN COOUSER cu ON cp.COO_CODE = cu.COO_CODE " + " WHERE cu.COO_CODE = ? ";
	...
	public List<CooperationDTO> cooperSearchName(String cooperName) {
		...
		try {
			...
			sql = "SELECT cu.COO_CODE, COO_NAME, COO_TEL, COO_MANAGER, TO_CHAR(WORK_START_DATE, 'YYYY-MM-DD') WORK_START_DATE, "
					+ " TO_CHAR(WORK_END_DATE,'YYYY-MM-DD') WORK_END_DATE, USER_CODE " + " FROM COOPERATION cp"
					+ " LEFT OUTER JOIN COOUSER cu ON cp.COO_CODE = cu.COO_CODE " + " WHERE COO_NAME = ? ";
	...
	public List<CooperationDTO> cooperSearchAll() {
		...
		try {
			...
			sql = "SELECT cp.COO_CODE, COO_NAME, COO_TEL, COO_MANAGER, TO_CHAR(WORK_START_DATE, 'YYYY-MM-DD') WORK_START_DATE, "
					+ " TO_CHAR(WORK_END_DATE,'YYYY-MM-DD') WORK_END_DATE, USER_CODE " + " FROM COOPERATION cp"
	...
	public CooperationDTO cooperSearchEmpCode(String cooperEmpCode) {
		...
		try {
			...
			sql = "SELECT COO_USER_CODE, COO_NAME, COO_TEL, COO_MANAGER, TO_CHAR(WORK_START_DATE, 'YYYY-MM-DD') WORK_START_DATE, "
					+ " TO_CHAR(WORK_END_DATE,'YYYY-MM-DD') WORK_END_DATE, USER_CODE " + " FROM COOPERATION cp"
					+ " LEFT OUTER JOIN COOUSER cu ON cp.COO_CODE = cu.COO_CODE " + " WHERE COO_USER_CODE = ?";
	...
	/*
	 * @Override // 이름 없어 보류 public List<CooperationDTO> cooperSearchEmpName(String
	 * cooperEmpName) { CooperationDTO dto = null; List<CooperationDTO> list = new
	 * ArrayList<>(); PreparedStatement pstmt = null; ResultSet rs = null; String
	 * sql;
	 * 
	 * try {
	 * 
	 * sql = "SELECT COO_CODE, COO_NAME, COO_USER_CODE, WORK_START_DATE, " +
	 * " WORK_END_DATE, USER_CODE" + " FROM COOPERATION cp" +
	 * " LEFT OUTER JOIN COOUSER cu ON cp.COO_CODE = cu.COO_CODE " +
	 * " WHERE COO_USER_NAME = ?";
	 * 
	 * pstmt = conn.prepareStatement(sql);
	 ...
	 * rs = pstmt.executeQuery(); // executeQuery() : 반환 타입이 ResultSet
	 * 
	 * if(rs.next()) { dto = new CooperationDTO();
	 * 
	 * dto.setCoo_code(rs.getInt("COO_CODE"));
	 * dto.setCoo_name(rs.getString("COO_NAME"));
	 * dto.setCoo_code(rs.getInt("COO_USER_CODE"));
